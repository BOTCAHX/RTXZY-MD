...
// Fungsi async pembantu khusus resolve LID via Baileys signalRepository
...
// Fungsi utama getJid (sync, tanpa async)
conn.getJid = (sender) => {
    ...
    // 1. Cache dulu (paling cepat)
    if (conn.isLid?.has(jid)) {
        const cached = conn.isLid.get(jid);
        if (cached?.endsWith('@s.whatsapp.net')) return cached;
    }

    // 2. Trigger resolve async di background (pakai fungsi async di atas)
    resolveLidToPn(conn, jid);  // ini jalan async, ga blokir return

    // 3. Fallback sync: cek metadata participants di semua chat
    ...
    // Kalau belum ada mapping → return LID dulu
    // Nanti setelah resolveLidToPn selesai (biasanya cepat setelah pesan masuk), cache terisi
    return jid;
};
