...
const findObject = (obj: unknown, key: string, value: unknown): WolfPlayer[] => {
    const result: WolfPlayer[] = [];
    const recursiveSearch = (obj: unknown) => {
        if (!obj || typeof obj !== "object") {
            return;
        }
        if ((obj as Record<string, unknown>)[key] === value) {
            result.push(obj as WolfPlayer);
        }
        Object.keys(obj).forEach(function(k) {
            recursiveSearch((obj as Record<string, unknown>)[k]);
        });
    };
    recursiveSearch(obj);
    return result;
...
// get data player
const dataPlayer = (sender: string, data: unknown): WolfPlayer => {
    let result = findObject(data, "id", sender);
    let index: WolfPlayer | false = false;
    if (result.length > 0 && result[0].id === sender) {
        index = result[0];
    } else {
        return index as unknown as WolfPlayer;
    }
    return index as unknown as WolfPlayer;
};

// get data player by id
const dataPlayerById = (id: number | string, data: unknown): WolfPlayer => {
...
// get player id
const getPlayerById = (from: string, sender: string, id: number | string, data: unknown): { index: number; sesi?: string; db: WolfPlayer } | false => {
    room = sesi(from, data);
    if (!room) return false;
...
const roleAmount = (from, data): WolfRoleAmount | false => {
    const result = sesi(from, data) as WolfRoom | false;
    if (!result) return false;
...
// get player win
const getWinner = (from, data): WolfWinner => {
    room = sesi(from, data);
    if (!room) return { voting: undefined, status: null };
    var ww = 0;
    var orang_baek = 0;
    for (let i = 0; i < room.player.length; i++) {
...
async function malam(conn, x, data) {
    ...
    if (hasil_vote === 0) {
        ...
        return conn.sendMessage(x.room, { image: { url: thumb3 }, caption: textnya })
            .then(() => {
                ...
                if (getWinner(x.room, data).status != null)
                    return win(sesi(x.room, data), 1, conn, data);
            });
        ...
        ""
            ...
            return await conn.sendMessage(x.room, { image: { url: thumb4 }, caption: textnya, mentions: ment })
                .then(() => {
                    changeDay(x.room, data);
                    voteDone(x.room, data);
                    resetVote(x.room, data);
...
async function win(x, t, conn, data) {
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        return await conn.sendMessage(x.room, { image: { url: thumb5 }, caption: textnya, mentions: ment })
            .then(() => {
                delete data[x.room];
            });
