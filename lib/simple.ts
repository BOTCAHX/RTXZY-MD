const exports: Record<string, unknown> & {
	attach: (conn: import('../types/connection.js').WaConnection & Record<string, unknown>) => import('../types/connection.js').WaConnection;
	smsg: (conn: unknown, m: unknown, hasParent?: boolean) => unknown;
	logic: (check: unknown, inp: unknown[], out: unknown[]) => unknown;
	protoType: () => void;
...
import { imageToWebp, videoToWebp, writeExifImg, writeExifVid } from '../lib/exif.ts'

import { toAudio, toPTT, toVideo } from './converter.ts'
...
export const WAMessageStubType: Record<string, number> = (proto.WebMessageInfo?.StubType || {}) as Record<string, number>;
...
const WMI = proto.WebMessageInfo as unknown as import('../types/proto.js').WaProtoWebMessageInfoClass;
const MSG = proto.Message as unknown as import('../types/proto.js').WaProtoMessageClass;
const ZAPO_ID_RE = /^3EB0[0-9A-F]{18}$/i;
const CONTENT_SKIP_KEYS = ['senderKeyDistributionMessage', 'messageContextInfo', 'protocolMessage'];

export function getContentType(message: Record<string, unknown> | null | undefined): string {
	const keys = Object.keys(message || {});
	return keys.find(k => !CONTENT_SKIP_KEYS.includes(k)) || keys[keys.length - 1] || '';
}
...
interface SendContent {
  ...
  ptt?: unknown;
  edit?: unknown;
  react?: { text?: string; key?: unknown };
  delete?: unknown;
  poll?: { name?: string; values?: unknown[]; selectableCount?: number };
  location?: { degreesLatitude?: number; degreesLongitude?: number; name?: string; address?: string; jpegThumbnail?: unknown };
  contacts?: { contacts?: Array<{ displayName?: string; vcard?: string }> };
  forward?: { message?: Record<string, unknown> };
  [key: string]: unknown;
}
...
export function generateForwardMessageContent(message, forceForward = true) {
	...
	const mtype = getContentType(protoMsg as Record<string, unknown>) || 'conversation';
...
export function generateWAMessageFromContent(jid: string, content: unknown, options: { userJid?: string; quoted?: { message?: unknown; key?: { id?: string } } } = {}) {
	return {
...
export async function generateWAMessageContent(content: Record<string, unknown>, options: Record<string, unknown> = {}) {
...
export async function generateWAMessage(jid: string, content: Record<string, unknown>, options: { userJid?: string } = {}) {
...
export async function resolveMedia(media) {
	...
	if (typeof media === 'string') {
		...
		if (fs.existsSync(media)) return media; // local file path
		return media;
	}
...
exports.attach = (conn) => {
 conn.isLid = new NodeCache({
    ...
    const seedOwnLid = () => {
        try {
            const creds = (conn._client?.getCredentials?.() || conn.authState?.creds || {}) as Record<string, unknown>;
            const ownLid = creds.meLid as string | undefined || (creds.me as { lid?: string } | undefined)?.lid;
            const ownJid = creds.meJid as string | undefined || (creds.me as { id?: string } | undefined)?.id;
            if (ownLid && ownJid) {
    ...
    conn.query = async (node) => {
        const n = node as { attrs?: Record<string, unknown> } | null | undefined;
        if (n?.attrs?.xmlns === 'w:profile:picture' && n.attrs.type === 'set') {
            const pic = getBinaryNodeChild(node, 'picture');
            const bytes = pic?.content;
            if (bytes) {
                return conn._client.profile.setProfilePicture(bytes instanceof Uint8Array ? bytes : new Uint8Array(bytes), n.attrs.target as string | undefined);
            }
            return undefined;
        }
    ...
    conn.downloadM = async (m, type, saveToFile) => {
        const media = m?.msg && (m.msg.url || m.msg.directPath || m.msg.mediaKey) ? m.msg : m;
        if (!media || !(media.url || media.directPath || media.mediaKey)) return Buffer.alloc(0);
	try {
		const wrapper = wrapMediaSource(media, type || m.mtype);
		const bytes = await conn._client.message.downloadBytes(wrapper);
		const buffer = Buffer.from(bytes);
		if (saveToFile) {
			const ft = await fileTypeFromBuffer(buffer);
			const filename = path.join('tmp', Date.now() + '.' + (ft?.ext || 'bin'));
    ...
    conn.relayWAMessage = async (message: Record<string, unknown>, options: Record<string, unknown> = {}) => {
    	return conn.relayMessage((message.key as { remoteJid?: string } | undefined)?.remoteJid || message.remoteJid as string, message.message || message, { ...options, messageId: (message.key as { id?: string } | undefined)?.id || options.messageId });
    };
    ...
    conn.sendMessage = async (jid: string, content: string | SendContent, options: string | Record<string, unknown> = {}) => {
    	if (typeof options === 'string') options = {}; 
    	const opts: Record<string, unknown> = { ...(options as Record<string, unknown>) };

    	let mentionJids = [...new Set([
    		...(typeof content === 'object' && content !== null && Array.isArray(content.mentions) ? content.mentions : []),
    		...(typeof content === 'object' && content !== null && Array.isArray((content.contextInfo as { mentionedJid?: unknown[] } | undefined)?.mentionedJid) ? (content.contextInfo as { mentionedJid?: unknown[] }).mentionedJid! : []),
    		...(Array.isArray(opts.mentions) ? opts.mentions : []),
    		...(Array.isArray((opts.contextInfo as { mentionedJid?: unknown[] } | undefined)?.mentionedJid) ? (opts.contextInfo as { mentionedJid?: unknown[] }).mentionedJid! : []),
	...
	} else if (content?.forward != null) {        const f = content.forward;
        const fmsg = f?.message || f || {};
        const fmtype = getContentType(fmsg as Record<string, unknown>) || 'conversation';
		const fcontent = (fmsg[fmtype] || {}) as Record<string, unknown>;
		const text = typeof fcontent === 'string'
			? fcontent
			: (fcontent.text || fcontent.caption || (fmsg as Record<string, unknown>).conversation || '');
		zapContent = { type: 'text', text: normText(String(text || '')), ...mentionCtx };
		sendOpts2 = { ...(sendOpts2 as Record<string, unknown>), forward: true } as typeof sendOpts & { forward?: boolean };
    ...
    conn.relayMessage = async (jid: string, message: unknown, options: Record<string, unknown> = {}) => {
...
conn.getJid = (sender) => {
    ...
    for (const chat of Object.values((conn.chats || {}) as Record<string, { metadata?: { participants?: Array<{ lid?: string; id?: string; jid?: string; phoneNumber?: string }> } }>)) {
...
conn.ev.on('lid-mapping.update', (updates) => {
    ...
    conn.waitEvent = (eventName: string, is: () => boolean = () => true, maxTries = 25) => {
  ...
  conn.delay = (ms) => new Promise((resolve) => setTimeout(resolve, ms))
    ...
    conn.resize = async (buffer: string | Buffer, uk1: number, uk2: number) => {
    	return new Promise<Buffer>(async(resolve, reject) => {
    		var baper = await jimp.read(buffer as string);
    		var ab = await baper.resize(uk1, uk2).getBufferAsync(jimp.MIME_JPEG)
    		resolve(ab)
    	})
    }
    
    conn.sendMedia = async (jid, path: string | Buffer, quoted: unknown, options: Record<string, unknown> = {}) => {
    ...
    conn.getFile = async (PATH, returnAsFilename) => {
        let res, filename
        let data = Buffer.isBuffer(PATH) ? PATH : /^data:.*?\/.*?;base64,/i.test(String(PATH)) ? Buffer.from(String(PATH).split(',')[1], 'base64') : /^https?:\/\//.test(String(PATH)) ? await (res = await fetch(String(PATH))).buffer() : fs.existsSync(String(PATH)) ? (filename = String(PATH), fs.readFileSync(String(PATH))) : typeof PATH === 'string' ? PATH : Buffer.alloc(0)
        if (!Buffer.isBuffer(data)) throw new TypeError('Result is not a buffer')
        let type = await fileTypeFromBuffer(data) || {
            mime: 'application/octet-stream',
            ext: '.bin'
        }
        if (data && returnAsFilename && !filename) (filename = path.join(__dirname, '../tmp/' + Date.now() + '.' + type.ext), await fs.promises.writeFile(filename, data))
        return {
            res,
            filename,
            ...type,
    ...
    conn.sendFile = async (jid: string, path: string | Buffer, filename = '', caption = '', quoted?: unknown, ptt: boolean | Record<string, unknown> = false, options: Record<string, unknown> = {}) => {
        ...
        let opt: Record<string, unknown> = { filename }
        if (quoted) opt.quoted = quoted
        if (!type) if (options.asDocument) options.asDocument = true
        let chat: Record<string, unknown> = global.db && global.db.data && global.db.data.chats && global.db.data.chats[jid] ? global.db.data.chats[jid] as Record<string, unknown> : {}
    ...
    conn.sendImageAsSticker = async (jid: string, path: Buffer | string, quoted?: unknown, options: Record<string, unknown> = {}) => {
        let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(String(path).split(',')[1], 'base64') : /^https?:\/\//.test(path) ? await (await fetch(String(path))).buffer() : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        let buffer
    ...
    conn.sendVideoAsSticker = async (jid: string, path: Buffer | string, quoted?: unknown, options: Record<string, unknown> = {}) => {
        let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(String(path).split(',')[1], 'base64') : /^https?:\/\//.test(path) ? await (await fetch(String(path))).buffer() : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        let buffer
        if (options && (options.packname || options.author)) {
...
VERSION:3.0
FN:${name.replace(/\n/g, '\\n')}
item1.TEL;waid=${number}:${PhoneNumberFn('+' + number).getNumber('international')}
item1.X-ABLabel:Ponsel${biz.description ? `
PHOTO;BASE64:${((await conn.getFile(await conn.profilePictureUrl(njid)).catch(_ => ({})) || {}) as { data?: { toString?: (enc?: string) => string } }).data?.toString?.('base64')}
X-WA-BIZ-DESCRIPTION:${(biz.description || '').replace(/\n/g, '\\n')}
X-WA-BIZ-NAME:${(String((((conn.chats[njid] || {}) || { vname: conn.chats[njid]?.name }).vname || await conn.getName(njid) || name)).replace(/\n/g, '\\n'))}
`.trim() : ''}
END:VCARD
`.trim()
    ...
    conn.reply = (jid: string, text = '', quoted?: unknown, options?: Record<string, unknown>) => {
    ...
    conn.decodeJid = (jid: string) => {
        if (!jid) return jid
        if (/:\d+@/gi.test(jid)) {
            let decode = jidDecode(jid) || {} as { user?: string; server?: string }
    ...
    conn.sendGroupV4Invite = async (jid: string, participant: string, inviteCode: string, inviteExpiration: string, groupName = 'unknown subject', caption = 'Invitation to join my WhatsApp group', options: Record<string, unknown> = {}) => {
        let msg = MSG.fromObject({
            groupInviteMessage: {
                inviteCode,
...
return await conn.sendMessage(jid, buttonMessage, {quoted})
    ...
    conn.sendH3Button = async (jid, content, displayText, link, displayCall, number, quickReplyText, id, quickReplyText2, id2, quickReplyText3, id3, quoted) => {
		let template = generateWAMessageFromContent(jid, MSG.fromObject({
			         templateMessage: {
             hydratedTemplate: {
     ...
     }), { userJid: conn.user.jid, quoted: quoted,     mentions: conn.parseMention(content + footer) } as Record<string, unknown>);
     return await conn.relayMessage(
         jid,
         template.message,
         { messageId: template.key.id }
    ...
    conn.cMod = async (jid: string, message: import('../types/proto.js').WaProtoWebMessageInfo, text = '', sender = conn.user.jid, options: Record<string, unknown> = {}) => {
        if (options.mentions && !Array.isArray(options.mentions)) options.mentions = [options.mentions]
        let copy = message.toJSON() as Record<string, unknown> & {
            message: Record<string, unknown>;
            key: Record<string, unknown> & { participant?: string; remoteJid: string; fromMe?: boolean };
            participant?: string;
        }
        delete copy.message.messageContextInfo
        delete copy.message.senderKeyDistributionMessage
        let mtype = getContentType(copy.message as Record<string, unknown>)
        let msg = copy.message
        let content = msg[mtype] as string | (Record<string, unknown> & { caption?: string; text?: string; contextInfo?: Record<string, unknown> }) | undefined
        if (typeof content === 'string') msg[mtype] = text || content
        else if (content?.caption) content.caption = text || content.caption
        else if (content?.text) content.text = text || content.text
        if (typeof content !== 'string' && content) {
            const merged: Record<string, unknown> = { ...content, ...options }
            merged.contextInfo = {
                ...(content.contextInfo || {}),
        ...
        return WMI.create(copy as Record<string, unknown>)
    }
    
    conn.cMods = (jid, message, text = '', sender = conn.user.jid, options = {}) => {
        let copy = (message as unknown as import('../types/proto.js').WaProtoWebMessageInfo).toJSON() as Record<string, unknown> & {
            message: Record<string, unknown> & { ephemeralMessage?: { message: Record<string, unknown> } };
            key: Record<string, unknown> & { participant?: string; remoteJid: string; fromMe?: boolean };
            participant?: string;
        }
        let mtype = getContentType(copy.message as Record<string, unknown>)
        ...
        let content = msg[mtype] as string | Record<string, unknown> | undefined
        if (typeof content === 'string') msg[mtype] = text || content
        else if (content?.caption) content.caption = text || content.caption
        else if (content?.text) content.text = text || content.text
    ...
    conn.copyNForward = async (jid, message, forwardingScore = true, options = {}) => {
        ...
        return m
            ...
            conn.fakeReply = async (jid: string, text = '', fakeJid = conn.user.jid, fakeText = '', fakeGroupJid?: string, options?: Record<string, unknown>) => {
    ...
    conn.loadMessage = conn.loadMessage || (async (messageID: string) => {
        return Object.entries((conn.chats || {}) as Record<string, { messages?: Record<string, { key?: { id?: string } }> }>)
            .filter(([, chat]) => typeof chat?.messages === 'object')
            .find(([, chat]) => Object.entries(chat.messages || {})
    ...
    conn.downloadM = async (m, type, saveToFile) => {
        ...
        if (!media || !(media.url || media.directPath || media.mediaKey)) return Buffer.alloc(0)
        try {
            return await conn.downloadContentFromMessage(media, type, saveToFile)
        } catch {
            return Buffer.alloc(0)
        }
    }
    ...
    conn.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
        let quoted = (message.msg ? message.msg : message) as WaMessage
	...
	let type = await fileTypeFromBuffer(buf)
        ...
        await fs.writeFileSync(trueFileName, buf)
        return trueFileName
    }
    ...
    conn.getName = async (jid = "", withoutContact = false): Promise<string> => {
    ...
    try {
        return PhoneNumberFn("+" + jid.replace("@s.whatsapp.net", "")).getNumber("international")
    } catch {
        return jid.split("@")[0]
    }
}
        ...
        if (!Array.isArray(m)) m = [m]
            for (const message of m) {
                ...
                if (!message) continue
                    ...
                    const mtype = getContentType(message.message as Record<string, unknown> | undefined)
                    const chat = conn.decodeJid(message.key.remoteJid || message.message?.senderKeyDistributionMessage?.groupId || '')
                    const msgContent = message.message?.[mtype] as { contextInfo?: { quotedMessage?: Record<string, unknown>; participant?: string; remoteJid?: string; stanzaId?: string; mentionedJid?: string[] } } | undefined
                    if (msgContent?.contextInfo?.quotedMessage) {
                    let context = msgContent.contextInfo
                    let participant = conn.decodeJid(context.participant || '')
                    const remoteJid = conn.decodeJid(context.remoteJid || participant)
                    let quoted = context.quotedMessage as Record<string, unknown>
                    if ((remoteJid && remoteJid !== 'status@broadcast') && quoted) {
                        ...
                        const qMtypeObj = quoted[qMtype] as Record<string, unknown>
                        let qCtx = qMtypeObj.contextInfo as Record<string, unknown> | undefined
                        if (!qCtx) qCtx = qMtypeObj.contextInfo = {}
                        qCtx.mentionedJid = context.mentionedJid || qCtx.mentionedJid || []
                        ...
                        if (isGroup && !participant) participant = remoteJid
                            ...
                            let qChats = conn.chats[participant] as Record<string, unknown>
                            if (!qChats) qChats = conn.chats[participant] = { id: participant, isChats: !isGroup }
                                if (!qChats.messages) qChats.messages = {}
                                    const qChatsMsgs = qChats.messages as Record<string, unknown>
                                    if (!qChatsMsgs[context.stanzaId as string] && !qM.key.fromMe) qChatsMsgs[context.stanzaId as string] = qM
                                        let qChatsMessages
                                        if ((qChatsMessages = Object.entries(qChatsMsgs)).length > 40) qChats.messages = Object.fromEntries(qChatsMessages.slice(30, qChatsMessages.length))
                                    }
                            }
    ...
    conn.getBuffer = async (url: string, options: Record<string, unknown> = {}) => {
        try {
            ...
            const raw = res.data as ArrayBuffer | Uint8Array | string
            return Buffer.from(raw instanceof Uint8Array ? raw : typeof raw === 'string' ? raw : new Uint8Array(raw))
        } catch (e) {
            // ignore fetch errors
            return Buffer.alloc(0)
        }
...
exports.smsg = (conn: import('../types/connection.js').WaConnection, input: unknown, hasParent?: boolean) => {
    if (!input) return input
    let M: import('../types/proto.js').WaProtoWebMessageInfoClass = WMI
    let m: import('../types/connection.js').WaMessage & Record<string, unknown> = M.create(input as Record<string, unknown>) as import('../types/connection.js').WaMessage & Record<string, unknown>
    
    if (m.key) {
        m.id = m.key.id
        // isZapo: message generated by the bot's own client (zapo id format: 3EB0 + 18 hex)
        m.isZapo = !!m.id && ZAPO_ID_RE.test(m.id)
        
        let chatJid = m.key.remoteJid;
        if (chatJid?.endsWith('@lid')) {
            // prefer the PN alternative (adapter fills remoteJidPn from key.remoteJidAlt)
        ...
        // Safer isGroup detection (primary fix)
        m.isGroup = 
        ...
        m.sender = conn.decodeJid(String(senderJid))
        m.fromMe = m.key.fromMe || areJidsSameUser(m.sender, conn.user.id)

        if (process.env.DEBUG_LID === '1' || global.opts?.['debug-lid']) {
            if (((exports as unknown as Record<string, number>)._lidDbgCount = ((exports as unknown as Record<string, number>)._lidDbgCount || 0) + 1) <= 30) {
            }
        }
    }

    if (m.message) {
        m.mtype = getContentType(m.message as Record<string, unknown> | undefined)
        m.msg = (m.message || {})[m.mtype as string] as WaMessage['msg']
        ...
        if (m.mtype == 'protocolMessage' && m.msg?.key) {
            const msgKey = m.msg.key as Record<string, unknown>
            if (msgKey.remoteJid == 'status@broadcast') msgKey.remoteJid = m.chat
            if (!msgKey.participant || msgKey.participant == 'status_me') msgKey.participant = m.sender
            msgKey.fromMe = conn.decodeJid(String(msgKey.participant)) === conn.decodeJid(conn.user.id)
            if (!msgKey.fromMe && msgKey.remoteJid === conn.decodeJid(conn.user.id)) msgKey.remoteJid = m.sender
            m.msg.key = msgKey as unknown as NonNullable<WaMessage['msg']>['key']
        }
        
        const _rawText = m.msg?.text || m.msg?.caption || m.msg?.contentText || (typeof m.msg === 'string' ? m.msg : '')
        m.text = String(_rawText || '')
        if (!m.text && m.msg && typeof m.msg === 'object') {
            const _dyn = m.msg as Record<string, unknown>
            m.text = String(_dyn.selectedDisplayText || ((_dyn.hydratedTemplate as Record<string, unknown> | undefined)?.hydratedContentText as string | undefined) || '')
        }
        
        m.mentionedJid = ((m.msg?.contextInfo as { mentionedJid?: string[] } | undefined)?.mentionedJid || []).map(jid => conn.getJid(jid) || jid)
        ...
        if (m.quoted) {
            ...
            if (m.quoted) {  
                ...
                quotedObj.id = m.msg?.contextInfo?.stanzaId
                quotedObj.chat = conn.decodeJid(String(m.msg?.contextInfo?.remoteJid || m.chat || m.sender))
                quotedObj.isZapo = !!quotedObj.id && ZAPO_ID_RE.test(String(quotedObj.id))
                quotedObj.sender = conn.getJid(String(m.msg?.contextInfo?.participant || '')) || conn.getJid(String(m.msg?.contextInfo?.remoteJid || '')) || m.sender
                ...
                quotedObj.fromMe = !!quotedObj.sender && !!_quotedMe && (areJidsSameUser(String(quotedObj.sender), _quotedMe) || conn.decodeJid(String(quotedObj.sender)) === conn.decodeJid(_quotedMe) || (!!_quotedMeLid && String(quotedObj.sender).split('@')[0] === String(_quotedMeLid).split('@')[0]))
                quotedObj.participant = m.msg?.contextInfo?.participant || (quotedObj.fromMe ? _quotedMe : quotedObj.sender)
                if (quotedObj.id && !quotedObj.fromMe) {
                    const _stored = conn.chats?.[conn.decodeJid(m.chat)]?.messages?.[String(quotedObj.id)]
                    if (_stored && typeof _stored.key?.fromMe === 'boolean') quotedObj.fromMe = _stored.key.fromMe
                }
                quotedObj.text = quotedObj.text || quotedObj.caption || quotedObj.contentText || ''

                quotedObj.name = ''
                const _qName = conn.chats && conn.chats[conn.decodeJid(String(quotedObj.sender))]
                if (_qName && _qName.name) quotedObj.name = _qName.name
                if (!quotedObj.name) conn.getName(String(quotedObj.sender)).then((n) => { if (n && !quotedObj.name) quotedObj.name = n }).catch(() => {})
                quotedObj.mentionedJid = ((quotedObj.contextInfo as Record<string, unknown> | undefined)?.mentionedJid as string[] | undefined || []).map(jid => conn.getJid(jid) || jid)

                let vM = quotedObj.fakeObj = M.create({
                    key: {
                ...
                } as unknown as import('../types/proto.js').WaProtoWebMessageInfo)
                
                m.getQuotedObj = m.getQuotedMessage = async () => {
                    if (!quotedObj.id) return null
                    let q = M.create((await conn.loadMessage(String(quotedObj.id)) || vM) as unknown as Record<string, unknown>)
                    return exports.smsg(conn, q)
                }
                

                quotedObj.download = (saveToFile = false) => conn.downloadM(quotedObj as unknown as WaMessage, String(quotedObj.mtype).replace(/message/i, ''), saveToFile)
            
                quotedObj.reply = (text: unknown, chatId: unknown, options?: unknown) => conn.reply(chatId ? String(chatId) : m.chat, String(text), vM, options as Record<string, unknown>)
                quotedObj.copy = () => exports.smsg(conn, M.create(M.toObject(vM)))
                quotedObj.forward = (jid: unknown, forceForward = false) => conn.forwardMessage(String(jid), vM, forceForward)
                quotedObj.copyNForward = (jid: unknown, forceForward = true, options = {}) => conn.copyNForward(String(jid), vM, forceForward, options)
                quotedObj.cMod = (jid: unknown, text = '', sender = quotedObj.sender, options = {}) => conn.cMod(String(jid), vM, String(text), String(sender), options)
                quotedObj.delete = () => conn.sendMessage(String(quotedObj.chat), { delete: (vM as import('../types/proto.js').WaProtoWebMessageInfo).key })
                m.quoted = quotedObj as unknown as WaMessage
            }
        }
    }
    ...
    m.download = (saveToFile = false) => conn.downloadM(m as WaMessage, String(m.mtype).replace(/message/i, ''), saveToFile)
    m.reply = (text: unknown, chatId: unknown, options?: unknown) => conn.reply(chatId ? String(chatId) : String(m.chat), String(text), m, options as Record<string, unknown>)
    m.copy = () => exports.smsg(conn, M.create(M.toObject(m as unknown as import('../types/proto.js').WaProtoWebMessageInfo)))
    ...
    m.cMod = (jid: unknown, text = '', sender = m.sender, options = {}) => conn.cMod(String(jid), m, String(text), String(sender), options)
    m.delete = () => conn.sendMessage(String(m.chat), { delete: (m as unknown as import('../types/proto.js').WaProtoWebMessageInfo).key })
...
exports.protoType = () => {
  ...
  ;(ArrayBuffer.prototype as unknown as { toBuffer: () => Buffer }).toBuffer = function () {
    return Buffer.from(new Uint8Array(this as unknown as ArrayBuffer))
  }
  ;(Uint8Array.prototype as unknown as { getFileType: () => Promise<unknown> }).getFileType =
    (ArrayBuffer.prototype as unknown as { getFileType: () => Promise<unknown> }).getFileType =
    (Buffer.prototype as unknown as { getFileType: () => Promise<unknown> }).getFileType = async function () {
      return await fileTypeFromBuffer(this as unknown as Uint8Array)
    }
  ;(String.prototype as unknown as { isNumber: () => boolean }).isNumber = (Number.prototype as unknown as { isNumber: () => boolean }).isNumber = isNumber
  ;(String.prototype as unknown as { capitalize: () => string }).capitalize = function () {
    return (this as unknown as String).charAt(0).toUpperCase() + (this as unknown as String).slice(1, (this as unknown as String).length)
  }
  ;(String.prototype as unknown as { capitalizeV2: () => string }).capitalizeV2 = function () {
    const str = (this as unknown as String).split(' ')
    return str.map(v => String(v).charAt(0).toUpperCase() + String(v).slice(1)).join(' ')
  }
  ;(String.prototype as unknown as { decodeJid: () => string }).decodeJid = function () {
    const self = this as unknown as string
    if (/:\d+@/gi.test(self)) {
      const decode = jidDecode(self) || {} as { user?: string; server?: string }
      return (decode.user && decode.server && decode.user + '@' + decode.server || self).trim()
    } else return self.trim()
  }
  ;(Number.prototype as unknown as { toTimeString: () => string }).toTimeString = function toTimeString() {
  ...
  ;(Number.prototype as unknown as { getRandom: typeof getRandom }).getRandom =
    (String.prototype as unknown as { getRandom: typeof getRandom }).getRandom =
    (Array.prototype as unknown as { getRandom: typeof getRandom }).getRandom = getRandom
}
...
export default exports as unknown as {
	attach: (conn: unknown) => import('../types/connection.js').WaConnection;
	smsg: (conn: unknown, m: unknown, hasParent?: boolean) => unknown;
};
