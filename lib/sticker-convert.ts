...
let _lottieQueue: Promise<unknown> = Promise.resolve()

function renderLottieFrames(lottieJson: LottieJson, { maxFrames = 60 }: { maxFrames?: number } = {}): Promise<LottieRenderResult> {
	const run = (): Promise<LottieRenderResult> => renderLottieFramesInner(lottieJson, { maxFrames })
	const p = _lottieQueue.then(run, run)
	_lottieQueue = p.catch(() => {})
	return p
}

async function renderLottieFramesInner(lottieJson: LottieJson, { maxFrames = 60 }: { maxFrames?: number } = {}): Promise<LottieRenderResult> {
	...
	return new Promise<LottieRenderResult>((resolve, reject) => {
