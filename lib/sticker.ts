...
import { ffmpeg } from './converter.ts'
...
async function sticker5(img: Buffer | string, url?: string | false | { packname?: string; author?: string; categories?: string[] }, packname?: string, author?: string, categories: string[] = ['']) {
  const opts = typeof url === 'object' && url !== null ? url : {}
  const stickerMetadata = {
    type: 'full' as const,
    pack: packname ?? opts.packname,
    author: author ?? opts.author,
    categories: categories.length ? categories : (opts.categories ?? ['']),
  }
  const input = img ? img : (typeof url === 'string' ? url : '')
  return await new WSF.Sticker(input, stickerMetadata as unknown as ConstructorParameters<typeof WSF.Sticker>[1]).build()
}
...
async function sticker(img: Buffer, url?: string, ...args: [string?, string?, string[]?]) {
    let lastError
    for (let func of [
      sticker3, // sticker5,
      ...
      try {
        let stiker = await (func as (img: Buffer, url?: string | false, ...rest: [string?, string?, string[]?]) => Promise<Buffer | string>)(img, url, ...args)
