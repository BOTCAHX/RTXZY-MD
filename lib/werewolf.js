...
async function malam(conn, x, data) {
    ...
    if (hasil_vote === 0) {
        ...
        let ment = [];
        return conn.sendMessage(m.chat, { image: { url: thumb3 }, caption: teextnya, mentions: [m.sender] }, { quoted: m });
            .then(() => {
                changeDay(x.room, data);
                voteDone(x.room, data);
                resetVote(x.room, data);
        ...
        ""
            ...
            return await conn.sendMessage(m.chat, { image: { url: thumb4 }, caption: teextnya, mentions: [m.sender] }, { quoted: m });
                .then(() => {
                    changeDay(x.room, data);
                    voteDone(x.room, data);
                    resetVote(x.room, data);
